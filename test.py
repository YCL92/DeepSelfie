...
class BasicModule(t.nn.Module):
    ...
    def load(self, root, device=None):
        ...
        return save_list[-1].split('_')[-1][:-4]
...
def main(args):
    ...
    weight_file = raw_process_model.load('./saves', device=device)
    raw_process_model.eval()
    args.output = os.path.join('.', weight_file)
    ...
    for file in file_list:
        ...
        with t.no_grad():
            ...
            pred_high,pred_low,pred_fused = raw_process_model(raw_data, amp_high, amp_low, cam_wb)

        # save to images
#         save_image(
#             pred_high.cpu().squeeze(),
#             os.path.join(args.output,
#                          '%s' % file.replace('.DNG', '-hi(%.2f).png'%amp_high.squeeze().item())))
#         save_image(
#             pred_low.cpu().squeeze(),
#             os.path.join(args.output,
#                          '%s' % file.replace('.DNG', '-lo(%.2f).png'%amp_low.squeeze().item())))
        save_image(
        ...
        if args.calib:
            ...
            img = cv2.imread(os.path.join(args.output,
                         '%s' % file.replace('.DNG', '-fuse.png')))
